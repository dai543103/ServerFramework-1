...
int ZmqBus::ZmqInit(uint64_t ullBusID, const char* pszAddress, enZmqSocketType eSocketType, enZmqProcType eProcType, bool bIsServerEnd)
{
    if(!pszAddress)
    {
	...
	m_ullServerBusID = ullBusID;
    m_iSocketType = eSocketType;
    m_iProcType = eProcType;
    m_iServiceType = bIsServerEnd ? EN_ZMQ_SERVICE_SERVER : EN_ZMQ_SERVICE_CLIENT;
...
uint64_t ZmqBus::GetBusID()
{
	return m_ullServerBusID;
}
