...
//获取当前ms时间
int64_t CTimeUtility::GetMSTime()
{
	m_stTimeValueTick.RefreshTime();

	return m_stTimeValueTick.GetMilliSec();
}

int CTimeUtility::ConvertUnixTimeToTimeString(time_t tTime, char* szTimeString, bool bSimpleFmt)
{
...
bool CTimeUtility::IsInSameWeek(time_t tTime1, time_t tTime2)
{
    ...
    //获取一周的截至时间,周一开始算新的周
    pTempTm->tm_mday += (7-pTempTm->tm_wday)%7;
    pTempTm->tm_hour = 23;
    pTempTm->tm_min = 59;
    pTempTm->tm_sec = 59;
