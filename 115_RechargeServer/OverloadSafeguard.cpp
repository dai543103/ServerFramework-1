...
int COverloadSafeguard::Timeout()
{
	...
	{
		//打印统计信息
		LOGDEBUG("===================COverloadSafeguard Stat===================\n");
		LOGDEBUG("RefuseAccept:%d, RefuseReceive:%d CloseConnect:%d\n",
            m_iRefuseAcceptNum, m_iRefuseReceiveNum, m_iCurNewCloseConnectNum);
		LOGDEBUG("CurUpPackNum:%d, DownPacketNum:%d \n", m_iCurUpPackCounter, m_iCurDownPacketCounter);
		LOGDEBUG("CurExternalConnect:%d, NewConnectNum:%d \n", m_iCurExternalConnectNum, m_iCurNewConnectNum);
		LOGDEBUG("==========================End Stat===========================\n");
	}
...
int COverloadSafeguardStrategy::Initialize(COverloadSafeguard* pstSafeguard, const char* pszStrategyCfg)
{
	...
	LOGDEBUG("COverloadSafeguardStrategy::Initialize, OpenSafeguard:%d, m_iConnectCoefficient:%d, "
	...
	for(i = 0; i < m_shStrategyNum; ++i)
	{
		...
		LOGDEBUG("Item[%d]: Value:%d, AcceptConnect:%d, AcceptReceive:%d, CloseOldConnect:%d\n", 
...
void COverloadSafeguardStrategy::CalculateStrategyVal()
{
	...
	LOGDEBUG("Calculate StrategyVal = %d, CurExternalConnectNum:%d, CurNewConnectNum:%d, CurUpPackCounter:%d\n", 
